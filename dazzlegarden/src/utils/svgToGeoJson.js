...
export function getSVGDimensions(svgNode) {
  ...
  if (
    ...
    if (!viewBox || typeof viewBox.replace(/^0-9/g, "") !== "number")
      return false;
...
export function svgToGeoJson(
  ...
  const elems = svgNode.querySelectorAll(
    "path, rect, polygon, circle, ellipse, polyline",
  );
  ...
  [].forEach.call(elems, (elem) => {
    ...
    const pathData = reducePathSegCurveComplexity(
      elem.getPathData({ normalize: true }),
      complexity,
    );
    ...
    geoJson.features.push({
      ...
      geometry: {
        ...
        coordinates:
          elem.tagName === "polyline" ? mappedCoords : [mappedCoords],
      },
    });
  });
